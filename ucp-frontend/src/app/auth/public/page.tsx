...
}, [isRegisterValid, isMessage, router]);
              ...
              <div className="relative flex items-center justify-center overflow-hidden px-4 py-8 min-h-[calc(100vh-78px)] bg-[radial-gradient(circle_at_80%_10%,rgba(247,247,248,0.685),transparent_30%)]">
                ...
                <div className="relative w-full max-w-md rounded-3xl overflow-hidden border border-slate-400/25 p-8 md:p-10 backdrop-blur-md bg-[#e0f5d6b9] shadow-[0_35px_85px_-28px_rgba(238,231,231,0.9)]">
                  ...
                  <div>
                    <header>
                        <section>
                            ...
                            onClick={handleBackAction}>retouner
                            </button>
                        </section>
                    </header>
                    </div>
                  ...
                  <div className="max-w-sm mx-auto">
                    ...
                    {
                      isMessage && (
                    <div className={`mt-5 ${isRegisterValid?'bg-green-500/10 border  text-black px-4' :'bg-red-500/10 border  text-black px-4'}  py-3 rounded-lg text-sm`}>
                        {message}
                      </div>
                      )
                  }
                    <form onSubmit={handleRegistration} className="mt-6 space-y-4">
                      ...
                      <div>
                        <select
                          className="w-full p-3 rounded-xl text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 transition-all focus:ring-emerald-400/60 focus:border-emerald-300 border border-[#ced1d1] bg-slate-800/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] appearance-none cursor-pointer"
                          value={type_entite}
                          onChange={(e) => setType_entite(e.target.value)}
                        >
                          <option value="" disabled className="bg-slate-800">
                            Sélectionnez votre type d entité
                          </option>
                          <option value="ENTREPRISE" className="bg-slate-800">Entreprise</option>
                          <option value="BUREAU_ETUDES" className="bg-slate-800">Bureau d’études</option>
                          <option value="ONG" className="bg-slate-800">ONG</option>
                          <option value="PARTICULIER" className="bg-slate-800">Particulier</option>
                          <option value="CONSULTANT" className="bg-slate-800">Consultant</option>
                        </select>
                      </div>
                      {/* NIF */}
                      <div>
