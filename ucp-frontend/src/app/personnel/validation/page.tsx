...
import ValidationModal from "@/app/personnel/demande-achat/components/ValidationModal";
...
export default function ValidationDashboardPage() {
  ...
  const validationStep = getValidatorStep(currentUser) as
    | DemandeAchat["etape_validation_actuelle"]
    | null;
  ...
  const validationQueryDemandeId = useMemo(() => {
    const raw = searchParams.get("demande");
    const parsed = raw ? Number(raw) : NaN;
    return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
  }, [searchParams]);
  const mineScopeHref = "/validation";
  const allScopeHref = "/validation?scope=all";
    ...
    () =>
      demandes.filter(
        (demande) =>
          ...
          (!validationStep ||
            demande.etape_validation_actuelle === validationStep),
      ),
    [demandes, validationStep],
  );
  const { filteredDemandes, filterProps } = useDashboardFilters(
    validationBaseDemandes,
  );
  ...
  const sections = useMemo<Record<SectionKey, SectionData>>(
    () => ({
      pending: {
        ...
        emptyText:
          "Aucun état de besoins en attente de validation. Belle journée !",
      },
    }),
    [orderedDemandes],
  );
  ...
  const searchResults = useMemo(
    () => (isSearching ? filterDemandesByQuery(orderedDemandes, query) : []),
    [isSearching, orderedDemandes, query],
  );
  ...
  const validationDemande = useMemo(
    () => demandes.find((item) => item.id === selectedValidationId) ?? null,
    [demandes, selectedValidationId],
  );
  ...
  const handleRunTableAction = (
    ...
    const actionUrl = new URL(action.href, window.location.origin);
    if (actionUrl.pathname.endsWith("/validation")) {
  ...
  useEffect(() => {
    if (!isHydrated || !validationQueryDemandeId) return;
    if (selectedValidationId === validationQueryDemandeId) return;

    setSelectedValidationId(validationQueryDemandeId);
    void loadFullDemande(validationQueryDemandeId);
  }, [isHydrated, validationQueryDemandeId, selectedValidationId]);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-0.5 text-[10px] font-black uppercase tracking-widest text-slate-400">
                  {validationRoleLabel || "Comité d'approbation"}
                </p>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
              ...
              <p className="text-sm text-slate-500">
                Votre file de traitement est vide. Vous recevrez une
                notification lorsqu&apos;un nouvel état de besoins nécessitera
                votre attention.
              </p>
            </div>
          ) : displayMode === "table" ? (
            ...
            <div className="animate-in slide-in-from-top-2 fade-in duration-300">
              <SearchResultsList
                ...
                onActionClick={(demande: DemandeAchat) =>
                  handleOpenValidation(demande.id)
                }
              />
            </div>
            ...
            <div className="space-y-4">
              <AccordionSection
                ...
                onToggle={() =>
                  setActiveSection(
                    activeSection === "pending" ? null : "pending",
                  )
                }
                currentUser={currentUser}
                onOpenDetail={(id: number) => handleOpenDetail(id)}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center flex-wrap gap-2 mb-2">
          ...
          {deadlineState &&
            (deadlineState.status === "ATTENTE_CRITIQUE" ||
              ...
              <span
                className={`flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border ${deadlineState.status === "ATTENTE_CRITIQUE" ? "bg-orange-50 text-orange-600 border-orange-100 animate-pulse" : "bg-slate-50 text-slate-500 border-slate-100"}`}
              >
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[11px] font-bold text-slate-400">
          ...
          <span className="text-slate-400 font-medium italic hidden sm:block truncate max-w-[300px]">
            Attendu{" "}
            {getElapsedLabel(demande.updated_at ?? demande.submitted_at)}
          </span>
        </div>
      </div>
