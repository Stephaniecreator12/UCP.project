...
from apps.users.services.permissions import TdrStRole, get_users_for_role


def _frontend_base_url() -> str:
    raw = getattr(settings, "FRONTEND_APP_URL", "") or getattr(settings, "FRONTEND_URL", "")
    return raw.rstrip("/") or "http://localhost:3000"


def _form_url(document: TdrStDocument) -> str:
    return f"{_frontend_base_url()}/TdrSt/formulaire?id={document.id}"


def _logo_url() -> str:
    return f"{_frontend_base_url()}/ucp-sante-logo-color.png"
...
def send_document_submitted_email(document: TdrStDocument) -> None:
    ...
    verificateurs = get_users_for_role(TdrStRole.VERIFICATEUR_TECHNIQUE)
...
def send_demande_final_approve_email(document: TdrStDocument) -> None:
    ...
    approbateurs = get_users_for_role(TdrStRole.APPROBATEUR_FINAL)
...
def send_ano_request_email(document: TdrStDocument) -> None:
    ...
    bailleurs = get_users_for_role(TdrStRole.BAILLEUR)
