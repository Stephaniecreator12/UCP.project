...
def get_dashboard_stats() -> dict:
    """Statistiques globales pour le dashboard (vision globale)"""
    now = timezone.now()
    ...
    # 2. Taux de validation (documents validés / total documents)
    ...
    # 3. Nombre de sources de financement uniques
    financement_sources_count = _count_unique_financement_sources()

    # 4. Documents par mois (année en cours)
    monthly_data = _get_monthly_documents_data(docs_year, current_year)

    # 5. Documents par type (TDR vs ST)
    documents_by_type = _get_documents_by_type()

    # 6. Documents par source de financement
    documents_by_source = _get_documents_by_source()

    # 7. KPIs
    kpis = _get_kpis(current_year, current_month)
...
def _count_unique_financement_sources() -> int:
    """Compte les sources de financement uniques"""
    sources_set = set()
...
def _get_monthly_documents_data(docs_year, current_year: int) -> list:
    """Génère les données mensuelles"""
...
def _get_documents_by_type() -> list:
    """Documents par type (TDR/ST)"""
...
def _get_documents_by_source() -> list:
...
def _get_kpis(current_year: int, current_month: int) -> dict:
    """Calcul des KPI"""
    ...
    avg_delay = _calculate_avg_validation_delay()
...
def _calculate_avg_validation_delay() -> float | None:
    """Calcule le délai moyen entre DEPOT et validation finale"""
