...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    function LoginPageContent() {
      ...
      const [email, setEmail] = useState(isEvaluationLogin ? evaluationEmail : validationEmail);
      const [password, setPassword] = useState("");
      const [showPassword, setShowPassword] = useState(false);
      ...
      const handleLogin = async (e: React.FormEvent) => {
        ...
        if (isEvaluationLogin && evaluationOffreId !== null) {
          ...
          try {
            await accessEvaluationByCode(evaluationOffreId, cleanEmail, cleanPassword);
            redirectTo(
              router,
              `/evaluation_offre/${evaluationOffreId}/evaluate?email=${encodeURIComponent(cleanEmail)}&code=${encodeURIComponent(cleanPassword)}`,
            );
            return;
          } catch (err) {
        ...
        const result = await login(cleanEmail, cleanPassword, setAccess);
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    placeholder={isValidationLogin || isEvaluationLogin ? "Saisir l'email reçu dans le mail" : "Saisir votre email"}
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={isValidationLogin ? "Saisir le mot de passe du mail" : isEvaluationLogin ? "Saisir le code d'accès du mail" : "Saisir votre mot de passe"}
                    />
                    <button
                      type="button"
